...
class TrainManager:
    ...
    def __init__(self, model: Model, config: dict, test=False) -> None:
        ...
        # Weights & Biases
        self.use_wandb = _HAS_WANDB and not test
        if self.use_wandb:
            wandb.init(
                project="ucu-text-to-sign",
                config=config,
                name=self.model_dir.split("/")[-1],
                mode="offline",
            )
    ...
    def train_and_validate(self, train_data, valid_data) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            self.logger.info('Epoch %3d: total training loss %.5f', epoch_no+1,
                             epoch_loss)
            if self.use_wandb:
                wandb.log({"train/epoch_loss": epoch_loss, "epoch": epoch_no + 1},
                          step=self.steps)
        else:
